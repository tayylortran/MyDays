        ...
        <KeyboardAvoidingView pointerEvents="box-none" behavior={Platform.OS === 'ios' ? 'padding' : undefined}
          ...
          <View style={styles.sheet} accessibilityViewIsModal>
            ...
            <View style={styles.controls}>
              <View style={styles.searchField}>
                ...
                <TextInput value={state.friendQuery} onChangeText={controller.setFriendQuery} editable={!busy}
                  accessibilityLabel="Search your friends" placeholder="Search your friends" placeholderTextColor="#a29d94"
                ...
                {!!state.friendQuery && <Pressable accessibilityRole="button" accessibilityLabel="Clear friend search" disabled={busy}
                  hitSlop={8} onPress={() => controller.setFriendQuery('')}><Ionicons name="close-circle" size={19} color="#a29d94" /></Pressable>}
              </View>
            ...
            <FlatList data={friends} keyExtractor={(friend) => friend.friendshipId}
              ...
              ListEmptyComponent={state.loading && !state.lists ? <ActivityIndicator color="#716d66" style={styles.empty} />
              ...
              renderItem={({ item }) => {
                ...
                return <View style={styles.person}>
                  ...
                  <Pressable accessibilityRole="button" accessibilityLabel={`Remove ${item.username} from friends`}
                    ...
                    {removing ? <ActivityIndicator color="#716d66" size="small" /> : <Text style={styles.removeText}>Remove</Text>}
                  </Pressable>
                </View>;
              }} />
          </View>
...
const styles = StyleSheet.create({
  ...
  removeButton: { minHeight: 44, minWidth: 70, paddingHorizontal: 12, alignItems: 'center', justifyContent: 'center', borderRadius: 23, backgroundColor: '#efede8' },
  removeText: { fontSize: 12, fontWeight: '600', color: '#716d66' },
  dimmed: { opacity: 0.5 },
  error: { color: '#a34736', fontSize: 13, lineHeight: 20, marginBottom: 8 },
  textButton: { minHeight: 44, justifyContent: 'center', paddingHorizontal: 8 },
  confirmation: { backgroundColor: '#f1eee8', borderRadius: 16, padding: 16, marginTop: 8, marginBottom: 12, gap: 8 },
  confirmTitle: { fontSize: 16, fontWeight: '600', color: '#292925' },
  ...
  confirmButton: { minHeight: 44, paddingHorizontal: 16, borderRadius: 23, justifyContent: 'center', backgroundColor: '#a34736' },
  confirmButtonText: { color: '#fffdfa', fontSize: 13, fontWeight: '600' },
  empty: { alignItems: 'center', paddingVertical: 48, gap: 8 },
  emptyTitle: { fontSize: 17, fontWeight: '500', color: '#716d66' },
});
