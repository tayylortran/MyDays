...
import { MAX_HANGOUT_PHOTOS, type Photo, type SaveHangoutInput, type SavedHangout } from './types';

const photoDirectory = new Directory(Paths.document, 'photos');
type PhotoRow = { id: string; hangout_id: string; uri: string; thumb_uri: string | null; sort: number; updated_at: number };
...
export async function saveHangoutWithPhotos(input: SaveHangoutInput): Promise<SavedHangout> {
  ...
  try {
    ...
    await transaction(db, async (tx) => {
      ...
      const ordered = [
        ...previous.filter((p) => retainedIds.has(p.id)).map((p): Photo => ({
          id: p.id, hangoutId: p.hangout_id, uri: p.uri, thumbUri: p.thumb_uri ?? undefined,
          sort: p.sort, updatedAt: p.updated_at,
        })),
        ...staged,
      ];
...
export async function deleteHangoutWithPhotos(id: string): Promise<void> {
  const db = await getDb();
  await transaction(db, async (tx) => {
